...
from model_utils import get_feature_coordinates, process_image_with_test_py, calculate_average_rgb
...
def result(filename):
    ...
    segmentation_mask = cv2.imread(f"/content/personalcolor_model/face-parsing.PyTorch/res/test_res/{filename}.png", cv2.IMREAD_GRAYSCALE)
    original_image = cv2.imread(f"/content/personalcolor_model/face-parsing.PyTorch/res/test_res/{filename}.jpg")
...
if __name__ == '__main__':
    #run_with_ngrok(app)  # Flask와 ngrok 연결
    app.run()
